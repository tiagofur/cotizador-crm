...
function buildPdf(doc: PDFKit.PDFDocument, data: {
  ...
  finish: string; notes: string | null; showPrices: boolean;
  ...
  /* ---------- Encabezado ---------- */
  doc.font('Helvetica-Bold').fontSize(16).fillColor('#1c1917').text(data.company.companyName || 'Cotizador de Muebles', left, 40);
  doc.font('Helvetica').fontSize(8.5).fillColor('#78716c');
  const contact = [data.company.companyAddress, data.company.companyPhone, data.company.companyEmail].filter(Boolean).join('  ·  ');
  if (contact) doc.text(contact, left, 60, { width: pageWidth * 0.6 });
  doc.font('Helvetica-Bold').fontSize(15).fillColor('#b45309').text('COTIZACIÓN', right - 150, 40, { width: 150, align: 'right' });
  doc.font('Helvetica').fontSize(9).fillColor('#44403c').text(`Folio: ${data.folio}`, right - 150, 60, { width: 150, align: 'right' });
  doc.text(fmtDate(data.date), right - 150, 73, { width: 150, align: 'right' });

  let y = 92;
  doc.moveTo(left, y).lineTo(right, y).lineWidth(1.5).strokeColor('#d6d3d1').stroke();
  y += 12;
  ...
  if (clientInfo.length) doc.font('Helvetica').fontSize(8.5).fillColor('#78716c').text(clientInfo.join('  ·  '), left, y + 28);

  const metaX = right - 220;
  doc.font('Helvetica-Bold').fontSize(9).fillColor('#78716c').text('ACABADO', metaX, y);
  doc.font('Helvetica-Bold').fontSize(10.5).fillColor(data.finish === 'MADERADO' ? '#4d7c0f' : '#44403c')
    .text(data.finish === 'MADERADO' ? 'MADERADO' : 'BLANCO', metaX, y + 13);
  doc.font('Helvetica').fontSize(8.5).fillColor('#78716c').text('Moneda: MXN', metaX, y + 28);
  y += 48;
  doc.moveTo(left, y).lineTo(right, y).lineWidth(0.75).strokeColor('#e7e5e4').stroke();
  y += 12;
  ...
  const drawHeader = () => {
    doc.rect(left, y, pageWidth, headerH).fill('#292524');
    doc.font('Helvetica-Bold').fontSize(8).fillColor('#fafaf9');
    let x = left + 6;
  ...
  for (const it of data.items) {
    ...
    const bg = zebra ? '#fafaf9' : '#ffffff';
    doc.rect(left, y, pageWidth, rowH).fill(bg);
    doc.rect(left, y, pageWidth, 0.5).fill('#e7e5e4');
    zebra = !zebra;
  ...
  if (showPrices) {
    ...
    doc.rect(bx - 8, y - 2, blockW + 8, 22).fill('#b45309');
    ...
    if (data.applyDistributor) {
      y += 6;
      doc.font('Helvetica-Bold').fontSize(9).fillColor('#4d7c0f')
      ...
      doc.rect(bx - 8, y - 2, blockW + 8, 20).fill('#4d7c0f');
  ...
  /* ---------- Notas y pie ---------- */
  doc.rect(left, 758, pageWidth, 0.75).fill('#e7e5e4');
  doc.font('Helvetica').fontSize(7.5).fillColor('#78716c');
  const notes = [
    data.notes || null,
    showPrices ? `Precios calculados con factor ${data.factor.toFixed(2)} sobre costos de materiales y herrajes.` : null,
    'Precios en pesos mexicanos (MXN).',
  ].filter(Boolean) as string[];
  notes.slice(0, 2).forEach((n, i) => {
    doc.text(n, left, 766 + i * 11, { width: pageWidth, lineBreak: false, height: 10, ellipsis: true });
  });
  doc.text('Página 1', right - 60, 766 + notes.slice(0, 2).length * 11 + 2, { width: 60, align: 'right', lineBreak: false });
}
